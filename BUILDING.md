...
* **N64Recomp**: <https://github.com/N64Recomp/N64Recomp>, cloned to
  ...
  -DCMAKE_BUILD_TYPE=Release && cmake --build build`), which produces both
  `build/N64Recomp` and `build/RSPRecomp`. The recorded build used upstream
  commit `ffb39cd` with no local changes.
...
The recompiler configs use paths relative to their own directory (N64Recomp
`src/config.cpp` joins every entry with the config file's parent directory;
RSPRecomp does the same in `read_config`), so the two inputs go in the port root
under fixed names. Both are ignored by git (`*.elf`, `*.z64`):
...
### 9. The audio microcode (already tracked; regenerate only if needed)

    ~/N64Recomp/build/RSPRecomp aspMain.us.toml

Writes `rsp/aspMain.cpp`, a C++ translation of the game's `aspMain` RSP audio
microcode (ROM 0x3E580, 0xE20 bytes, loaded at IMEM 0x1080). The output is
tracked, so this step is only needed after changing `aspMain.us.toml`. See
`NOTICE.md` about that file's origin.
...
`CMakeLists.txt` globs `RecompiledFuncs/funcs_*.c` into the `recomp_funcs`
static library, compiles `RecompiledPatches/patches.c` (if present) and
`rsp/aspMain.cpp` into the executable, links RT64 statically and links against
the SDL2 built from `lib/SDL`. The result is
...
tools, the patch sources, `rsp/aspMain.cpp` and (once the orchestrating commit
lands) the two symbol files. It does **not** contain:
