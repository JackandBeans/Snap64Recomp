...
Eye passes and the live handheld viewfinder use RT64's matched world matrices and vertex/UV interpolation weights at the runtime refresh rate. Cart snapshots are associated with game workloads and interpolated over the same interval, with shortest-path yaw and course-block origin rebasing. Tracking remains predicted each display frame, including the viewfinder's lens pose and zoom. The original game's photograph capture/scoring pass retains its simulation snapshot; it does not consume the private live viewfinder target. Desktop interpolation settings are preserved.
...
- Refresh-rate follow-up: VR also bypasses the workload queue's desktop deadline drops, mirror-driven frame reduction, and truncation when a newer simulation tick arrives. OpenXR's frame wait was counted as render cost, and normal tick arrival could discard remaining headset interpolation samples. Interpolation continues to follow the runtime's application cadence, including refresh changes and runtime throttling. Logs now distinguish measured submitted FPS from that cadence. A 22-second boot/title check on the connected 120 Hz headset at 3760x3996 per eye recovered to repeated 119.8–120.2 submitted FPS samples; the final 600-frame sample dropped zero interpolation frames. The preceding old-build check stayed mostly at a 60 Hz application cadence. Startup still reprojected, and sustained in-course 120 FPS has not been established by this check.
- Live viewfinder follow-up: the private handheld screen now replaces the simulation camera with the current tracked lens pose/zoom and uses the same interpolated world matrices and vertex/UV weights as the eyes. It also uses the live replay's full viewport/depth mapping. The original photograph/scoring renderer remains separate. A held-camera Beach preview at 90 Hz captured three distinct viewfinder images per 30 Hz game tick; seven within-tick image comparisons passed and a captured image was visually inspected. Release build and VR interaction tests pass; physical-controller smoothness still needs a headset check. `SNAP_VR_VIEWFINDER_TEST=1` with `--vr-preview` and `SNAP_VR_MODEL_TEST=1` saves twelve successive `vr-viewfinder-*.png` images and logs their source workload, weight, and lens position after the camera is held.
