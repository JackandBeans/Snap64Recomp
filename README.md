...
| `vr` | `false` | draw for a VR headset through OpenXR ("The headset"); `SNAP_VR=1` or `0` in the environment overrides it |
| `vr_world_scale` | `80.0` | the game's units per metre, for the eyes' separation and the head's movement; 20 to 400 |
| `vr_render_scale` | `1.0` | the eye pictures' size against what the runtime recommends; 0.5 to 2 |
| `vr_screen_width` | `2.4` | the menu screen's width in metres, three metres ahead of the seat; 1 to 8 |
...
Built on a Quest 3 over a Link cable and proved as far as this machine
allows without one: with the setting on and no headset connected the port
names the runtime, says no headset is attached and runs the scoring replay
flat with the same 45 photos as with the setting off. The eyes, the plane,
the screen and the controllers have not been seen in a headset yet; the
first run in one is what settles them, and the log says what each frame
carried. The hands, a camera held in one of them, apples thrown by hand
and the ZERO-ONE around you are the next stage.
