...
Run under WSL from the repo root:
  python3 tools/harvest_menu_font.py <rom> [--emit [--out <json path>]]

--emit writes the preview sheet and a JSON of every face and kern table for
comparing against the port's run-time harvest (src/menu_harvest.cpp, dumped
by SNAP_STATS as menu_font_runtime.json). Nothing is written into the tree:
the default --out is under build-win/, which git ignores. The port carries
no ROM pixels; this script is a reference, not a generator.
"""
import os
import sys
...
def main():
    ...
    if emit:
        from extract_menu_sprite import write_png
        ...
        for r in rows_png:
            ...
            for _ in range(S):
                big.append(line)
        os.makedirs("build-win/Release/menu_text_reference", exist_ok=True)
        ...
        def face(table):
            return {c: (wdt, cs, ce, [[(i, a) for (i, a) in row] for row in cell])
                    for c, (wdt, cs, ce, cell) in table.items()}

        out_path = "build-win/Release/menu_text_reference/menu_font_harvest.json"
        for i, arg in enumerate(sys.argv):
            if arg == "--out" and i + 1 < len(sys.argv):
                out_path = sys.argv[i + 1]
        os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
        with open(out_path, "w") as f:
            json.dump({"body": face(glyphs), "hdr": face(hdr), "crd": face(crd), "hlp": face(hlp),
                       "hlp_kern": [[a, b, gap] for (a, b), gap in sorted(hlp_kern.items())],
                       "hlp_space_kern": [[a, b, gap] for (a, b), gap in sorted(hlp_space_kern.items())]},
                      f)
        print("wrote", out_path, "(%d+%d+%d+%d glyphs, %d+%d kern pairs)" %
              (len(glyphs), len(hdr), len(crd), len(hlp), len(hlp_kern), len(hlp_space_kern)))
