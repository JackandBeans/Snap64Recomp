...
| `dxc` | **No license file in the tree.** The directory is rt64's `dxc-bin` repository at `cc15e71` (`VENDORING.md`), 17 DirectX Shader Compiler binaries and headers; the headers cite "LICENSE.TXT", which is not present. Which release each file is from was established by SHA-256 on 2026-09-02: `bin/x64/dxc-linux`, `lib/x64/libdxcompiler.so`, `libdxil.so` and `inc/*.h` are the files of Microsoft's release **v1.8.2403.2** (`linux_dxc_2024_03_29.x86_64.tar.gz`); `bin/x64/dxc.exe` and `dxcompiler.dll` (1.7.0.4147) are a **private build** of upstream commit `0dc8d9060` (2023-10-09) that matches no release; the arm64 and macOS binaries are private or LunarG builds; dxc-bin's `bin/x64/dxil.dll` is the file of release v1.7.2212 and **is not used** (next sentence). The compiler and headers are under the LLVM Release License (University of Illinois/NCSA): the release archives carry it as `LICENSE-LLVM.txt`, and their READMEs assign it to "all other files"; that text is tracked as `licenses/DirectXShaderCompiler.txt` (upstream `LICENSE.TXT` of 2026-09-02, whose first 43 lines are the release's file and whose remainder lists LLVM's third-party notices) and shipped in the package. **`dxil.dll`**, Microsoft's DXIL validator, is under `LICENSE-MS.txt`, Microsoft Software License Terms, which every release README assigns to it. The v1.7.2212 text is a time-limited pre-release agreement with no right to distribute, so the `dxil.dll` this port builds with and ships is the file of the next release, **v1.7.2308** (`dxc_2023_08_14.zip`; `tools/fetch_deps.py` fetches it and records the SHA-256), whose text carries a "Distributable Code" section (the same text as every later release up to v1.9.2607, checked 2026-09-02): the object code may be copied and distributed with an application that adds significant primary functionality, provided distributors and end users are bound by terms that protect Microsoft at least as much as that agreement and the distributor indemnifies Microsoft, and it may not be distributed in a way that puts it under a licence requiring source disclosure. That text is tracked verbatim as `licenses/DirectXShaderCompiler-dxil.txt` and shipped in the package; `dxil.dll` is a separate proprietary file placed beside the GPL-licensed executable, not part of it, and this port's licence does not extend to it. This is a reading of the files, not legal advice. | yes: `dxc.exe` compiles the shaders at build time; `dxcompiler.dll` and `dxil.dll` are loaded at run time and must sit beside the executable |
...
| **OpenXR-SDK** 1.1.63 | Apache License 2.0 (`lib/OpenXR-SDK/LICENSE`, The Khronos Group); the headers carry the same licence or the MIT | <https://github.com/KhronosGroup/OpenXR-SDK>, tag `release-1.1.63`, commit `f2448a8797c85814aa892efc1ab8707900fbcc78` | `lib/OpenXR-SDK`, ignored, pristine clone; the loader is built as a static library into the Windows executable (src/vr_openxr.cpp); Linux builds do not use it |
| jsoncpp (in `OpenXR-SDK/src/external/jsoncpp`) | MIT, or public domain where recognised (`lib/OpenXR-SDK/src/external/jsoncpp/LICENSE`) | <https://github.com/open-source-parsers/jsoncpp>, as bundled by the OpenXR SDK | three source files compiled into the loader |
...
ROM. The executable that is distributed does: it is compiled from the C that
N64Recomp writes out of the game's own instruction stream (`RecompiledFuncs/`,
generated from the builder's dump, never tracked), together with the RSP
audio microcode's translation below, which is how every N64Recomp port is
built and shipped. The player's dump is then read at every start as well,
for the game's data. Two more things the port needs are derived from the
game; both are produced from the builder's or the player's own copy, at
build time or at run time, and never enter the repository:
...
* `patches/src/*.c` -- replacement functions for the game, built the way
  every N64Recomp project builds them: a patch is a modified copy of the
  decompiled function it replaces, compiled against the decompilation's
  headers, and its origin is the game. The largest is `render_patch.c`,
  which reproduces `renPrepareModelMatrix` and `ren_func_80013C5C` from the
  decomp's `src/sys/render.c` with the matrix-group tags added
  (`patches/README.md`); the title, options, intro, fade and volume patches
  are smaller copies of the same kind.
...
| Asset | License | Origin | In this tree |
| --- | --- | --- | --- |
| Roboto Regular (bitmaps of eleven capitals) | Apache License 2.0 (`licenses/roboto.txt`) | The Roboto Project Authors, <https://github.com/googlefonts/roboto> | `src/snap_station_osd_font.h`, generated by `tools/osd_font_gen.py` for the Snap Station printer's on-screen lettering; no font file is tracked or shipped |
| `menu_text/recomp_logo.png` | this project's (GPLv3) | drawn for the port | the "Recomp" badge on the title screen |
| `src/snap64.ico` | this project's (GPLv3) | composed from `docs/logo.png` by `tools/icon_gen.py` | the executable's icon, compiled in through `src/snap64.rc.in` |
| `docs/screenshots/*.png` | **not this project's**: thirty-eight frames of Pokémon Snap as the port renders it (Nintendo, Creatures, GAME FREAK, HAL Laboratory, The Pokémon Company) | captured by me from the 1.0.0 build and the releases after it | the README, `docs/SCREENSHOTS.md` and the release pages only; not in the archive and not used by the program |
| `docs/logo.png` | **not this project's**: the "Snap" lettering, the starburst, the film strip and the film canister with its Poké Ball are the Pokémon Snap logo's (Nintendo, Creatures, GAME FREAK, The Pokémon Company), the "64" is the Nintendo 64 logo's; "Recomp" is the port's own word set in the same style | renderings of the two logos, composed by me | the README header and `docs/dev/social-preview.png` only; it names the game the port runs and is not used in the program. Their owners' trademarks; the README's first paragraph states the non-affiliation |
