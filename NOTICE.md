...
| `docs/screenshots/*.png` | **not this project's**: nineteen frames of Pokémon Snap as the port renders it (Nintendo, Creatures, GAME FREAK, HAL Laboratory, The Pokémon Company) | captured from the 1.0.0 build by the port's author | the README, `docs/SCREENSHOTS.md` and the release page only; not in the archive and not used by the program |
| `docs/logo.png` | **not this project's**: the "Snap" lettering, the starburst, the film strip and the film canister with its Poké Ball are the Pokémon Snap logo's (Nintendo, Creatures, GAME FREAK, The Pokémon Company), the "64" is the Nintendo 64 logo's; "Recomp" is the port's own word set in the same style | renderings of the two logos, composed by the port's author | the README header and `docs/dev/social-preview.png` only; it names the game the port runs and is not used in the program. Their owners' trademarks; the README's first paragraph states the non-affiliation |
