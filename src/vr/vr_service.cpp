...
bool input(uint16_t* buttons,float* x,float* y) {
    if(!requested.load()||(preview&&!std::getenv("SNAP_VR_NAME_TEST")&&!std::getenv("SNAP_VR_POINTER_TEST")))return false;
    ...
    if(s.pulses){s.pulseHeld|=s.pulses;s.pulseUntil=now+std::chrono::milliseconds(50);s.pulses=0;}
