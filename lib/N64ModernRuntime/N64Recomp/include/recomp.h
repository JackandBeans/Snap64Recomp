...
#ifdef _MSC_VER
// MSVC folds (int16_t)LO16(sum) into an untruncated displacement add, dropping
// the mod-0x10000 wrap the MIPS HI16/LO16 pair depends on (addresses land 64KB
// off when the low halves carry). A volatile barrier on the sum forces exact
// truncation semantics.
static inline uint32_t recomp_reloc_sum(int32_t sa, uint32_t off) {
    volatile uint32_t s = (uint32_t)sa + off;
    return s;
}
#undef RELOC_HI16
#undef RELOC_LO16
#define RELOC_HI16(section_index, offset) \
    HI16(recomp_reloc_sum(section_addresses[section_index], (offset)))
#define RELOC_LO16(section_index, offset) \
    LO16(recomp_reloc_sum(section_addresses[section_index], (offset)))
#endif
